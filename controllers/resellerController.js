...
const userSignUp = async (req, res) => {
    try {
        ...
        const { fullName, email, username, password, phoneNumber, address} = req.body;
        if(!fullName || !email || !username || !password || !phoneNumber || !address ){
            return res.status(400).json(`Please enter all fields.`)
        }
        const emailExist = await resellerModel.findOne({ email: email.toLowerCase() });
        if (emailExist) {
...
const verifyEmail = async (req, res) => {
    try {
        ...
        const user = await resellerModel.findOne({ email:email.toLowerCase() });
...
const userLogin = async (req, res) => {
    try {
        ...
        const existingUser = await resellerModel.findOne({email: email.toLowerCase()});
        if (!existingUser) {
            return res.status(404).json({
                message: "User not found."}); }
...
const makeAdmin = async(req, res)=> {
    try {
        ...
        const user = await resellerModel.findById(userId)
        if(!user){
...
module.exports ={
    userSignUp, verifyEmail, resendVerificationEmail, userLogin, resetPassword, forgotPassword, changePassword, makeAdmin, getOneUser, getAllUsers, userLogOut
}