...
const userSignUp = async (req, res) => {
    try {
        ...
        const emailExist = await supplierModel.findOne({ email: email.toLowerCase() });
        if (emailExist) {
...
const verifyEmail = async (req, res) => {
    try {
        ...
        const user = await supplierModel.findOne({ email:email.toLowerCase() });
...
const userLogin = async (req, res) => {
    try {
        ...
        const existingUser = await supplierModel.findOne({email: email.toLowerCase()});
        if (!existingUser) {
            return res.status(404).json({
                message: "User not found."}); }
